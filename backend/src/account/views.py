...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def validate(self, request):
        ...
        if len(validation_code) == 0:
            return Response({'detail':'The property is empty.'},status=status.HTTP_400_BAD_REQUEST)
        
        # Validate the code in the DB
        if validation_code != user_instance.validation_code:
            return Response({'detail':'The validation code is incorrect.'},status=status.HTTP_400_BAD_REQUEST)
